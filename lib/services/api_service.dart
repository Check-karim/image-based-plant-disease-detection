  ...
  Future<String> sendMessageGPT({required String diseaseName}) async {
    try {
      ...
      if (dioError.response != null) {
        return 'Dio Error: ${dioError.response?.data["error"]?["message"] ?? "Unknown Error"}';
      } else if (dioError.type == DioExceptionType.connectionTimeout) {
        return 'Connection Timeout Error: Please check your internet connection.';
      } else if (dioError.type == DioExceptionType.receiveTimeout) {
        return 'Receive Timeout Error: The server took too long to respond.';
      } else {
        return 'Unexpected Error: ${dioError.message}';
      }
    } on HttpException catch (httpError) {
      return 'HTTP Error: ${httpError.message}';
    } catch (error) {
